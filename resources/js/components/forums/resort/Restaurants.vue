...
<template>
    <div class="h-full p-4 rounded-lg shadow-md">
        <Link :href="`/forums/categories/travel/${continentSlug}/${countrySlug}/${resortSlug}/travel/posts`">
            <h2 class="mb-4 text-2xl font-bold text-center text-amber-200 hover:text-white">Food</h2>

            <div v-if="restaurantPosts.length === 0" class="text-center text-gray-400">
                No travel posts found.
            </div>

            <ul v-else class="space-y-4">
                <li
                    v-for="post in restaurantPosts"
                    :key="post.id"
