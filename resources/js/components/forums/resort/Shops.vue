...
<template>
    <div class="h-full p-4 rounded-lg shadow-md">
        <Link :href="`/forums/categories/travel/${continentSlug}/${countrySlug}/${resortSlug}/shops/posts`">
            <h2 class="mb-4 text-2xl font-bold text-center text-emerald-200 hover:text-white">Shops</h2>

            <div v-if="shopsPosts.length === 0" class="text-center text-gray-400">
                No shops posts found.
            </div>

            <ul v-else class="space-y-4">
                <li
                    v-for="post in shopsPosts"
                    :key="post.id"
